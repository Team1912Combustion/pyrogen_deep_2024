...
public class Stampede {
    ...
    public PinpointSensor pinpoint = null;
    ...
    public AngleTrackerIMU angleTracker = null;
    ...
    public void initWheelHardware(boolean withEncoder) {
        ...
        driveFrontLeft = setUpEncoderMotor(DriveConstants.front_left_name, DcMotor.Direction.FORWARD, 12, 10, 0.0, 5.0, withEncoder);
        driveFrontRight = setUpEncoderMotor(DriveConstants.front_right_name, DcMotor.Direction.FORWARD, 12, 10, 0.0, 5.0, withEncoder);
        driveRearLeft = setUpEncoderMotor(DriveConstants.back_left_name, DcMotor.Direction.REVERSE, 12, 10, 0.0, 5.0, withEncoder);
        driveRearRight = setUpEncoderMotor(DriveConstants.back_right_name, DcMotor.Direction.REVERSE, 12, 10, 0.0, 5.0, withEncoder);
    ...
    public void init(HardwareMap ahwMap) {
        ...
        if (true) {
            pinpoint = hwMap.get(PinpointSensor.class, "pinpoint");
            pinpoint.initialize();
            pinpoint.setEncoderResolution(
                    GoBildaPinpointDriver.GoBildaOdometryPods.goBILDA_4_BAR_POD);
            pinpoint.setOffsets(
                    Constants.OdometryConstants.xOffset,
                    Constants.OdometryConstants.yOffset);
            pinpoint.setEncoderDirections(
                    Constants.OdometryConstants.xDirection,
                    Constants.OdometryConstants.yDirection);
            Pose2d newPose = new Pose2d(yFieldPos, -1.*xFieldPos, Rotation2d.fromDegrees(headingField));
            pinpoint.setPose2d(newPose);
        }
    ...
    public void updateFieldPosition() {
        ...
        if (odopodLeft == null && otos == null && pinpoint == null) {
            ...
            // if we don't have wheel encoders or odopods or otos, we must have a pinpoint
            Pose2d currPos = pinpoint.getPose2d();
            yFieldPos = currPos.getX();
            xFieldPos = -1. * currPos.getY();
            headingField = currPos.getRotation().getDegrees();
        }
    ...
    public void setOrientation(double heading) {
        if (angleTracker != null) {
            angleTracker.setOrientation(heading);
        }
        if (pinpoint != null) {
            Pose2d pose = pinpoint.getPose2d();
            Pose2d newPose = new Pose2d(pose.getX(), pose.getY(),
    ...
    public double getOrientation() {
        if (angleTracker != null) {
            return angleTracker.getOrientation();
        }
        if (pinpoint != null) {
    ...
    public void reportTelemetry(Telemetry telemetry){
        ...
        if(pinpoint != null){
            Pose2d pos = pinpoint.getPose2d();
            telemetry.addData("Pinpoint Pose", "x: %4.2f, y: %4.2f, heading: %4.2f",
                    pos.getX(), pos.getY(), pos.getHeading()*180./Math.PI);
        }
        if (angleTracker != null) {
            double totalRotationInDeg = angleTracker.getOrientation();
            telemetry.addData("Heading", "%5.2f", ((totalRotationInDeg % 360) + 360) % 360);
        }
        telemetry.addData("Field Pose", "x: %4.2f, y: %4.2f, heading: %4.2f",
